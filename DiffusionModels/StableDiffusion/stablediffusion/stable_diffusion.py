...
class Text2Image:
    ...
    def generate(
        ...
        unconditional_tokens = np.array(_UNCONDITIONAL_TOKENS)[
            None].astype("int32")
        unconditional_tokens = np.repeat(
            unconditional_tokens, batch_size, axis=0)
...
def get_models(img_height, img_width, download_weights=True):
    ...
    # Create text encoder
    input_word_ids = Input(shape=(MAX_TEXT_LEN,), dtype="int32")
    input_pos_ids = Input(shape=(MAX_TEXT_LEN,), dtype="int32")
    embeds = CLIPTextTransformer()([input_word_ids, input_pos_ids])
    text_encoder = Model([input_word_ids, input_pos_ids], embeds)

    # Creation diffusion UNet
    context = Input((MAX_TEXT_LEN, 768))
    t_emb = Input((320,))
    latent = Input((n_h, n_w, 4))
    unet = UNet()
    ...
    latent = Input((n_h, n_w, 4))
    decoder = Decoder()
    decoder = Model(latent, decoder(latent))

    if download_weights:
        text_encoder_weights_fpath = get_file(
        ...
        diffusion_model_weights_fpath = get_file(
        ...
        decoder_weights_fpath = get_file(
