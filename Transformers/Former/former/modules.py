...
class SelfAttention(nn.Module):
    ...
    def __init__(self, emb, heads=8, mask=False):
        ...
        """
        super().__init__()
        assert emb % heads == 0, f'Embedding size ({emb}) must be divisible by the number of heads ({heads})' 
        ...
        s = emb // heads
        # - We will break the embedding matrix into 'heads' chunks and feed each other to a different attention head.
    ...
    def forward(self, x):
        b, t, e = x.size()
        h = self.heads
        assert e == self.emb, f'Expected {self.emb} embeddings, got {e}'
...
class SelfAttentionRelative(nn.Module):
    ...
    def __init__(self, emb, pos_embedding, heads=8, mask=False, ):
        ...
        self.parma, self.parmb = nn.Parameter(torch.randn(
            1, h, 1, s)), nn.Parameter(torch.randn(1, h, 1, s))
    ...
    def forward(self, x):
        ...
        positions = self.pos(torch.arange(2*t-1, device=d(x))
                             )[None, :].expand(b, 2*t-1, e)
        ...
        dot_tp = torch.einsum('bis, bjs -> bij', queries,
                              keys_pos)  # -- token with position
        dot_tp = util.slice_diag(dot_tp, l=t)
        assert dot_tp.size() == (b*h, t, t), f'{dot_tp.size()}'

        dot_pt = torch.einsum('bis, bjs -> bij', parma,
                              keys)  # -- position with token
        assert dot_pt.size() == (b*h, t, t), f'{dot_pt.size()}'

        dot_pp = torch.einsum('bis, bjs -> bij', parmb,
                              keys_pos)  # -- pos with pos
