...
def test_padding(instance):
    ...
    for i in range(len(input_size)):
        ...
        x = np.random.randn(
            input_size[i], input_size[i], ch_num[i]).astype(np.float32)
...
def test_conv_nopad(instance, method1, method2, print_time=False,
    ...
    for i in range(len(input_size)):
        ...
        x = np.random.randn(
            input_size[i], input_size[i], ch_num[i]).astype(np.float32)
        f = np.random.randn(f_num[i], f_size[i],
                            f_size[i], ch_num[i]).astype(np.float32)
        b = np.random.randn(f_num[i]).astype(np.float32)

        print('x:{} f:{} b:{}'.format(x.shape, f.shape, b.shape))

        # par_conv_gl_coal_nopad_block1
        result1 = method1(x, f, b, pad=0, stride=stride[i])
        ...
        if print_time == True:
            print('method1:{}{}:{}{}:{} '.format('\n',
                                                 str(whole_time1), getattr(
                                                     instance, whole_time1),
                                                 str(kernel_time1), getattr(instance, kernel_time1)))
            print('method2:{}{}:{}{}:{} '.format('\n',
                                                 str(whole_time2), getattr(
                                                     instance, whole_time2),
...
def zf_net_conv(instance, par_conv_method, atol=2e-1, rtol=2e-2):
    ...
    out_layer1 = test_conv_pool(instance, par_conv_method,
                                ...
                                pool_size=3, pool_stride=2, pool_pad=1, atol=atol, rtol=rtol)
...
def test_conv_pool(instance, par_conv_method,
    ...
    ser_conv = instance.ser_conv(
        inter_result, filter_w, filter_b, stride=conv_stride)
    par_conv = par_conv_method(
        inter_result, filter_w, filter_b, stride=conv_stride)
    try:
